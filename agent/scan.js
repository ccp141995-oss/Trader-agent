...
function buildAgentPrompt(signals, mtfData, alignments){
  ...
  const system = "You are JARVIS's trading research sub-agent for a Hyperliquid perpetuals account, acting as a master chart technician. "
    ...
    + `For suggested_size_pct_equity and suggested_leverage: this account's configured maximums are ${MAX_POSITION_PCT}% of equity and ${MAX_LEVERAGE}x leverage — `
    + "these are hard caps enforced separately regardless of what you suggest, so there is no benefit to suggesting conservatively low numbers out of general caution. "
    + "Scale your suggestion to reflect your actual conviction in *this specific idea* relative to that maximum: your highest-conviction setups (strong multi-factor "
    + "confluence, clean technical structure, aligned trend) should suggest sizing at or near the maximum; only scale meaningfully below it for a setup you'd "
    + "genuinely trade smaller yourself — a mediocre or borderline idea, not every idea by default. Do not suggest a token-sized position (e.g. 1-2% of a much "
    + "higher maximum) for an idea you otherwise describe as high-conviction — that's an inconsistent signal. "
